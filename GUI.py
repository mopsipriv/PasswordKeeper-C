...
def mainApp():
    ...
    def add_info():
        ...
        def save_and_encrypt():
            ...
            pass_buffer = ctypes.create_string_buffer(password.encode('utf-8'))
            c_lib.xor_encrypt_decrypt(pass_buffer)
            encrypted_data = str(pass_buffer.value) 
            ...
            print(f"Encrypted (C result): {encrypted_data}")
    ...
    def search_info():
        ...
        info_btn = Button(search_window, text="Search",
    ...
    def view_decrypted_password():
        ...
        buffer_size = len(encrypted_bytes)
        pass_buffer = ctypes.create_string_buffer(encrypted_bytes, buffer_size)

        c_lib.xor_encrypt_decrypt(pass_buffer)
        
        try:

            decrypted_password = pass_buffer.value.decode('utf-8')
        except UnicodeDecodeError:
            messagebox.showerror("Error", "Decryption failed or resulting data is corrupt.")
            return


        view_window = Toplevel(main_window)
        view_window.title("Decrypted Password")
        view_window.geometry("450x180")
    ...
    style.configure("Treeview",
                    ...
                    font=("Arial", 10))
    style.map("Treeview",
              background=[('selected', SUCCESS_COLOR)],
